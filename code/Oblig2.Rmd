...
```{r}
wei.lung<-survreg(Surv(Y,C) ~ Treat + PS + Age + Prior + factor(Cell), data = lungcancer.df, dist = "weibull")
```
...
## c)
```{r}
#fit.coxph <- survreg(Surv(Y,C) ~ Treat + PS + Age + Prior + factor(Cell), data = lungcancer.df, dist = "weibull")
#fit.coxph
